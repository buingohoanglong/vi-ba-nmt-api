...
def train_model(model, opt):
    ...
    model.train()
    start = time.time()
    # print('opt inside train_model', opt)
    if opt.checkpoint > 0:
        cptime = time.time()
    # list_train = list(opt.train)
    # print("opt.train & len =>", list_train, len(list_train))
    # print("opt.train[0]", list_train[0], list_train[1])
    # shouldStopEpochs = False
    for epoch in range(opt.epochs):
        print("training ep %d - trainlen: %d"%(epoch,opt.train_len))
        total_loss = 0
        ...
        for i, batch in enumerate(opt.train): 
            ...
            if i % 10 == 0:
                ...
                print(">>> time: [%s] training ep: %d - batch(i): %d , loss: %s"%(now, epoch, i, total_loss))
...
def main():
    ...
    parser.add_argument('-SGDR', type=int, default=0)
    ...
    # print("opt.device:", opt.device, torch.cuda.is_available())
    opt.device = 0
    # if opt.device == 0:
    ...
    print("len(SRC.vocab):", len(SRC.vocab))
    print("len(TRG.vocab):", len(TRG.vocab))
